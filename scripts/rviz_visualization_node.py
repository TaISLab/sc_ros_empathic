...
The human joint angles are NOT drawn here: for a live view of q1..q4
use one rqt_plot window --
    rqt_plot /shared_control_node/diag/joint_deg/data[0]:data[1]:data[2]:data[3]
(degrees). ~diag/joint_rho / joint_margins give the normalised
quantities the joint-safety efficiency is computed from.

The human arm is NOT drawn here: the visuo-tactile pipeline already
publishes it (/skeleton_3d/keypoints point cloud, /skeleton_3d/connectors
marker, /right_arm_description URDF). rviz/shared_control.rviz has
displays for those.
